sSampler <- nimbleFunction(
  ...
  setup = function(model, mvSaved, target, control) {
    ...
    y1.nodes <- model$expandNodeNames(paste("y1[",i,",",1:J1,"]"))
    y2.nodes <- model$expandNodeNames("y2")
    pd1.nodes <- model$expandNodeNames(paste("pd1[",i,",",1:J2,"]"))
    pd2.nodes <- model$expandNodeNames(paste("pd2[",i,",",1:J2,"]"))
  ...
  run <- function() {
    ...
    if(z==0){ #propose from prior
      ...
      if(inbox){
        ...
        model$calculate(pd1.nodes) #update pd1 nodes
        lp_proposed_y1 <- model$calculate(y1.nodes) #get proposed y1 logProb
        #before updating pd2 nodes, divide out initial 1-pd2[i,] from nondetect.probs
        nondetect.probs.proposed <- nondetect.probs.initial/(1-model$pd2[i,])
        model$calculate(pd2.nodes) #update pd2 nodes
        #now multiply nondetect.probs by proposed 1-pd2[i,]
        nondetect.probs.proposed <- nondetect.probs.proposed*(1-model$pd2[i,])
